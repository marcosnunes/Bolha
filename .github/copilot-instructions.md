...
- **File uploads**: No hard size limit - automatic compression/optimization
...
### 2.1. **Media Compression Strategy** (CRITICAL)
Location: `src/components/CreatePostForm.jsx`, `EditProfileModal.jsx`, `HomePage.jsx`, `CadastroPage.jsx`

**Images > 100MB:**
- Compressed **client-side** using Canvas API before upload
- Algorithm: `Math.sqrt(maxSize / fileSize)` calculates scale factor
- Progressive quality reduction (0.9 → 0.3) until < 100MB
- Max dimensions for profile pics: 1024x1024px
- Output format: JPEG with quality optimization
- User feedback: Shows original → compressed size

**Videos (ANY size):**
- Upload directly to Cloudinary (no client-side limit)
- Server-side compression via Cloudinary transformation parameters:
  - `quality: 'auto:low'` - automatic quality optimization
  - `video_codec: 'h264'` - universal codec
  - `audio_codec: 'aac'` - optimized audio
  - `fetch_format: 'auto'` - best format per browser
- Extended timeout: 10 minutes (vs 5 min for images)
- User feedback: "⏳ Vídeo grande detectado. Será otimizado automaticamente."

**Implementation Pattern:**
```javascript
// Images: Canvas compression
const compressImage = (file, maxSizeMB = 100) => {
  // Create canvas, scale dimensions, reduce quality progressively
  const scaleFactor = Math.sqrt(maxSizeBytes / file.size);
  // Return compressed File object
}

// Videos: Cloudinary server-side
formData.append('quality', 'auto:low');
formData.append('video_codec', 'h264');
formData.append('audio_codec', 'aac');
xhr.timeout = 600000; // 10 min for videos
```

**Key Files:**
- `CreatePostForm.jsx` - Post media compression
- `EditProfileModal.jsx` - Profile pic compression
- `HomePage.jsx` - Quick profile pic update
- `CadastroPage.jsx` - Registration profile pic
...
- **Profile pics**: Auto-compressed (1024x1024 max), uploaded to Cloudinary, stored in `/profiles/{uid}/photoURL`
- **Post media**: Auto-compressed/optimized, uploaded with `resource_type` detection (image vs video)
...
- **No size limits**: Images compressed client-side, videos compressed server-side (see section 2.1)
