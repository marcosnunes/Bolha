...
// =====================================================
// PERSPECTIVE API - Moderação de Conteúdo
// =====================================================

const PERSPECTIVE_API_KEY = process.env.PERSPECTIVE_API_KEY;
const PERSPECTIVE_API_URL = 'https://commentanalyzer.googleapis.com/v1alpha1/comments:analyzeComment';

// Função para validar texto com Google Perspective API
// Detecta toxicidade em português E inglês
exports.validateTextWithPerspective = onCall({
  region: 'us-central1',
  timeoutSeconds: 30
}, async (request) => {
  try {
    ...
    if (!PERSPECTIVE_API_KEY) {
      logger.warn('PERSPECTIVE_API_KEY não configurada, permitindo texto por segurança');
      return {
        isSensitive: false,
        reason: 'API não configurada (fail-open)'
      };
    }

    const fetch = require('node-fetch');

    // Fazer requisição para Perspective API
    const perspectiveResponse = await fetch(PERSPECTIVE_API_URL, {
      method: 'POST',
      headers: {
    ...
    if (!perspectiveResponse.ok) {
      const errorData = await perspectiveResponse.json();
      logger.error('Erro Perspective API:', errorData);
      return {
        isSensitive: false,
    ...
    // Definir limites para cada atributo
    const thresholds = {
      TOXICITY: 0.7,
      SEVERE_TOXICITY: 0.5,
      IDENTITY_ATTACK: 0.6,
    ...
    for (const [attribute, threshold] of Object.entries(thresholds)) {
      if (scores[attribute]) {
        ...
        scoreDetails[attribute] = score;

        if (score > threshold) {
          isSensitive = true;
          flaggedCategories.push(`${attribute} (${(score * 100).toFixed(1)}%)`);
        }
      }
    }

    logger.info(`Texto validado - Sensível: ${isSensitive}, Categorias: ${flaggedCategories.join(', ')}`);

    return {
      isSensitive,
      flaggedCategories,
      scores: scoreDetails,
      reason: isSensitive ? `Conteúdo flagrado em: ${flaggedCategories.join(', ')}` : 'Conteúdo apropriado'
    };

  } catch (error) {
    logger.error('Erro ao validar com Perspective API:', error);
    return {
      isSensitive: false,
      reason: 'Erro na validação (fail-open)'
    };
  }
});
